function [BinaryMat] = binarycyl3D(Nx, Ny, Nz, InitC, EndC, Radii)

% ex 1cyl
% [BinaryMat] = binarycyl3D(64, 64, 64, [0,0,0], [64,64,64], 4);
...
% [BinaryMat] = binarycyl3D(Nx, Ny, Nz, InitC, EndC, Radii);
...
xinit = InitC(count,:); %init coordinates
xend = EndC(count,:);% end coordinates
radius=Radii(count);% radius of cylinder
...
figure, volshow(BinaryMat(:,:,:), 'BackgroundColor', 'white', 'Colormap', repmat([1,0,0], [256,1]));%, pbaspect(Nx, Ny, Nz)
